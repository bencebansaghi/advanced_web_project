...
const ModifyCard: React.FC<ModifyCardProps> = ({ card_id, onChange }) => {
    ...
    const modifyCard = async () => {
        try {
            ...
            const body: { card_id: string; title?: string; description?: string; color?: string } = { card_id };
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            />
            <Button variant="contained" onClick={modifyCard}>Submit</Button>
            <Button variant="contained" onClick={onChange}>Cancel</Button>
        </Box>
    );
};
